...
import com.epam.jwd.finalProject.dao.connection.ProxyConnection;
...
public enum ThreadLocalTransactionManager implements TransactionManager {
    ...
    private static final ThreadLocal<TransactionId> THREAD_CONNECTION = ThreadLocal.withInitial(() -> {
        return new SimpleTransactionId(new ProxyConnection(ConnectionPool.locking().getConnection(), ConnectionPool.instance()));
    });
